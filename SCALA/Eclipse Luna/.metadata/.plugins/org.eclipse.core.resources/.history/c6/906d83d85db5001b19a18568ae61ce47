...
object Colecciones extends App {
 ...
 /* def Generar_Tablero(n:Int): ParSeq[Int]={
  ...
  def Imprimir_Fila(l:ParSeq[Int]): Unit ={
   ...
   def Imprimir_matriz_XxY_aux(l:Iterator[List[Int]],n:Int): Unit = n match {
     case Y => println()
     case _ => Imprimir_Fila(l.next().par );println();Imprimir_matriz_XxY_aux(l,n+1)
    } 
   
  def Imprimir_matriz_XxY(l:List[Int]): Unit ={
      val lista_group =  l.grouped(X)
      Imprimir_matriz_XxY_aux(lista_group,0)
    }   
 ...
 /*--------------------------------Bucle de Juego -------------------------------*/
    ...
    def main(): Unit ={
      ...
      val tablero = Generar_Tablero(X*Y).toList //Inicializamos la lista
